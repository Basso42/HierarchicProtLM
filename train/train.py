...
def train_per_protein(
    ...
    if load_last_finetuned_weights:
        ...
        if filtered_paths:
            ...
            model, tokenizer = load_model(f"{path_weights}/{latest_epoch_weights}", num_labels=num_labels, loss_type=loss_type, loss_weights=loss_weights)
        else:
...
os.system(f"mkdir -p {sys.argv[2]}/train/SingleTaskClassif_Checkpoints_{loss_type}")
save_path = sys.argv[2] + f'/train/SingleTaskClassif_Checkpoints_{loss_type}/ankh_finetuned.pth'
